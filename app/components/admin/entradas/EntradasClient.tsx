...
import {
  Plus, Search, RefreshCw, ArrowDownToLine, AlertCircle, Package,
  ChevronLeft, ChevronRight, X, Eye, Trash2, CheckCircle2, Filter,
} from "lucide-react";
import { useEntradas } from "@/app/hooks/useEntradas";
...
function DetalleModal({ entrada, onClose }: { entrada: EntradaResponse; onClose: () => void }) {
  return (
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
        ...
        <div className="mb-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl" style={{ backgroundColor: "rgba(11,61,145,0.08)" }}>
              <ArrowDownToLine size={18} style={{ color: "#0b3d91" }} />
            </div>
            <div>
        ...
        <div className="mb-4 space-y-2">
          <div className="flex justify-between text-sm" style={{ fontFamily: "var(--font-poppins), sans-serif" }}>
            ...
            <span className="font-medium max-w-[60%] text-right" style={{ color: "#101828" }}>{entrada.descripcion}</span>
          </div>
          {entrada.proveedorNombre && (
            <div className="flex justify-between text-sm" style={{ fontFamily: "var(--font-poppins), sans-serif" }}>
              <span style={{ color: "#667085" }}>Proveedor</span>
              <span className="font-medium" style={{ color: "#101828" }}>{entrada.proveedorNombre}</span>
            </div>
          )}
          {entrada.confirmadaPor && (
            <div className="flex justify-between text-sm" style={{ fontFamily: "var(--font-poppins), sans-serif" }}>
              <span style={{ color: "#667085" }}>Confirmado por</span>
              <span className="font-medium" style={{ color: "#16a34a" }}>{entrada.confirmadaPor}</span>
            </div>
          )}
          {entrada.motivoRechazo && (
            <div className="rounded-2xl p-3" style={{ backgroundColor: "#fef2f2" }}>
              <p className="text-xs font-medium" style={{ color: "#b42318", fontFamily: "var(--font-poppins), sans-serif" }}>
                Motivo de rechazo: {entrada.motivoRechazo}
              </p>
            </div>
          )}
        </div>
          ...
          <div className="space-y-2">
            {entrada.detalles.map((d) => (
              <div key={d.id} className="flex items-center justify-between rounded-2xl px-4 py-3" style={{ backgroundColor: "#f9fafb" }}>
                ...
                <span className="text-sm font-semibold" style={{ color: "#0b3d91", fontFamily: "var(--font-poppins), sans-serif" }}>
  ...
  return (
    <div className="flex items-center justify-between border-t px-2 pt-4" style={{ borderColor: "#f0f0f4" }}>
      <span className="text-xs" style={{ color: "#9ca3af", fontFamily: "var(--font-poppins), sans-serif" }}>
        {totalElements} entrada{totalElements !== 1 ? "s" : ""} · Página {page + 1} de {totalPages}
      </span>
...
export default function EntradasClient() {
  const { data, loading, saving, error, successMsg, page, setPage, estadoFilter, filtrarEstado, reload, eliminar, clearMessages } = useEntradas();
  const [search, setSearch] = useState("");
  const [detalleEntrada, setDetalleEntrada] = useState<EntradaResponse | null>(null);
  ...
  const kpis = {
    ...
    rechazadas: entradas.filter(e => e.estado === "RECHAZADA").length,
  };
  ...
  return (
    <section className="flex flex-col gap-6 pb-10">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm" style={{ color: "#667085", fontFamily: "var(--font-poppins), sans-serif" }}>
              Solicitudes de ingreso al almacén — flujo ADMIN → BODEGA.
            </p>
          </div>
          <Link href="/entradas/add" className="inline-flex items-center gap-2 rounded-2xl px-4 py-2.5 text-sm font-semibold text-white transition"
      ...
      <div className="rounded-3xl border" style={{ borderColor: "#eaecf0", backgroundColor: "#fff", boxShadow: "0 2px 20px rgba(15,23,42,0.05)" }}>
        ...
        {loading && !data ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            <ArrowDownToLine size={40} style={{ color: "#d1d5db" }} />
            <p className="text-sm" style={{ color: "#9ca3af", fontFamily: "var(--font-poppins), sans-serif" }}>Sin entradas para mostrar</p>
          </div>
        ) : (
                ...
                {filtered.map(entrada => (
                  <tr key={entrada.id} style={{ borderBottom: "1px solid #f9fafb" }}
                    ...
                    <td className="px-5 py-4">
                      <span className="rounded-full px-2 py-0.5 text-xs font-semibold" style={{ backgroundColor: "rgba(11,61,145,0.08)", color: "#0b3d91" }}>
                        {entrada.detalles.length} ítem{entrada.detalles.length !== 1 ? "s" : ""}
                      </span>
                    </td>
                    <td className="px-5 py-4 text-sm" style={{ color: "#667085", fontFamily: "var(--font-poppins), sans-serif" }}>
                      {entrada.proveedorNombre ?? "—"}
                    </td>
                    <td className="px-5 py-4"><EstadoBadge estado={entrada.estado} /></td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <button onClick={() => setDetalleEntrada(entrada)} title="Ver detalle"
                        ...
                        {entrada.estado !== "CONFIRMADA" && (
                          <button onClick={() => setDeleteId(entrada.id)} title="Eliminar"
                            className="flex h-8 w-8 items-center justify-center rounded-xl border transition"
                            style={{ borderColor: "#e5e7eb", color: "#374151" }}
      ...
      {/* Modals */}
      <AnimatePresence>
        {detalleEntrada && <DetalleModal entrada={detalleEntrada} onClose={() => setDetalleEntrada(null)} />}
        {deleteId !== null && (
